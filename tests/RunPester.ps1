...
Set-Location "C:\OPEN_PROJECTS\ElmerPS\"
...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\ElmerPS\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\ElmerPS\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\ElmerPS\$Directory\*.ps1 -PassThru -ExcludeTag SkipThis
